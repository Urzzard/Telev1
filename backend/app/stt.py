...
# ========== GETTER CON SELECTOR ==========
def get_stt():
    """Retorna instancia única - selecciona entre Whisper y Faster Whisper según variable"""
    global _instance
    ...
    if _instance is None:
        if use_faster:
            ...
            logger.info(">>> Usando Whisper (openai-whisper) <<<")
...
class FasterWhisperSTT:
    ...
    def _es_texto_valido(self, texto: str) -> bool:
        """Filtra alucinaciones - misma lógica que SpeechToText"""
        if not texto or len(texto) < 2:
            return False
